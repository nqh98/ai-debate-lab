...
- **Telemetry:** `agent_call` carries the resolved model for an agent with a pinned model; omits the key for a CLI agent with no `models_command` (`model is None`); omits it on a failed attempt; a `fast` vote call and a `deep` propose call against a roster with a `models_command` record *different* models, which is the routing assertion the field exists for.
- **`Reply`:** an adapter returning a bare string is a type error at the boundary, not a silent `model=None` (the reason for changing the ABC).
